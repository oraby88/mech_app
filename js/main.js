...
const imageRow = document.querySelector('.hero-brands');

// Auto-scroll logic
function autoScroll() {
  const scrollAmount = 1; // Pixels to scroll each step
  const scrollInterval = 10; // Milliseconds between each scroll
  let currentPosition = 0;

  setInterval(() => {
    currentPosition += scrollAmount;

    if (currentPosition >= imageRow.scrollWidth - imageRow.clientWidth) {
      currentPosition = 0; // Reset to the beginning when reaching the end
    }

    imageRow.scrollLeft = currentPosition;
  }, scrollInterval);
}
